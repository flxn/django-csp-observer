...
def update_rules(force=False):
    if not force:
        last_update = app_settings.get_stored(app_settings.LAST_RULE_UPDATE, default=None)
    ...
    if rule_database.last_update < last_update:
    ...
    app_settings.put_stored(app_settings.LAST_RULE_UPDATE, time.time())
